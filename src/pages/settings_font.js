...
export function makeSettingsTabContentFont() {
	...
	addAsChildren(tabContent, [
		...
		textToNode(`<h2>${t('ui:font.FontMetrics')}</h2>`),
		textToNode(`<h3>${t('ui:font.KeyMetrics')}</h3>`),
		...
		textToNode(`<h3>${t('ui:font.OtherMetrics')}</h3>`),
		...
		textToNode(`<h2>${t('ui:font.Links')}</h2>`),
		...
		textToNode(`<h2>${t('ui:font.SVGFonts')}</h2>`),
